...
import { createInterceptingTransport, InterceptingRequestHandler } from './interceptor';
import type { RequestHandler } from './types';

// Mock fetch for tests
const originalFetch = globalThis.fetch;

describe('InterceptingRequestHandler', () => {
	let mockProxied: RequestHandler;
	...
	describe('request logging', () => {
		it('should log requests when logger is provided', async () => {
			...
			vi.mocked(mockProxied.handle).mockResolvedValue(mockResponse);

			const requestHandler = new InterceptingRequestHandler(undefined, logger, mockProxied);
		...
		it('should not log when logger is not provided', async () => {
			...
			const requestHandler = new InterceptingRequestHandler(undefined, undefined, mockProxied);
	...
	describe('response interception', () => {
		it('should call interceptor with request body and response', async () => {
			...
			const requestHandler = new InterceptingRequestHandler(interceptor, undefined, mockProxied);
	...
	describe('error handling', () => {
		it('should log errors when network request fails', async () => {
			...
			vi.mocked(mockProxied.handle).mockRejectedValue(networkError);
			...
			await expect(requestHandler.handle(request)).rejects.toThrow('Network error');
		...
		it('should rethrow network errors', async () => {
			...
			await expect(requestHandler.handle(request)).rejects.toThrow('Connection refused');
		});

		it('should handle non-Error exceptions', async () => {
			const logger = vi.fn();
			vi.mocked(mockProxied.handle).mockRejectedValue('string error');
			...
			await expect(requestHandler.handle(request)).rejects.toBe('string error');
...
describe('createInterceptingTransport', () => {
	...
	describe('with interceptor', () => {
		...
		it('should throw generic RPC error when no message', async () => {
			...
			await expect(config.request({ method: 'eth_call', params: [] })).rejects.toThrow('RPC Error');
		});
...
describe('import/export test', () => {
	it('should export InterceptingRequestHandler', () => {
		expect(InterceptingRequestHandler).toBeDefined();
	});
